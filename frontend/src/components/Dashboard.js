...
const Dashboard = () => {
  ...
  const handleDeleteHotel = async (hotelId, hotelName) => {
    if (!window.confirm(`"${hotelName}" otelini silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`)) {
      return;
    }
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Otel silinirken hata oluştu');
    }
  };

  const handleDeleteRoom = async (roomId, roomName) => {
    if (!window.confirm(`"${roomName}" salonunu silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`)) {
      return;
    }
    ...
    try {
      ...
      toast.error(error.response?.data?.detail || 'Salon silinirken hata oluştu');
    }
  };

  const handleDeleteBallroom = async (ballroomId, ballroomName) => {
    if (!window.confirm(`"${ballroomName}" balo salonunu silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`)) {
      return;
    }

    try {
      await axios.delete(`${API}/ballrooms/${ballroomId}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      
      toast.success('Balo salonu başarıyla silindi');
      fetchDashboardData(); // Refresh data
    } catch (error) {
      console.error('Delete ballroom error:', error);
      toast.error(error.response?.data?.detail || 'Balo salonu silinirken hata oluştu');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Admin Approval Panel - For Admins and Hotel Managers */}
        {(user.role === 'admin' || user.role === 'hotel_manager') && (
          <div className="mb-8">
            <AdminApprovalPanel onApprovalComplete={fetchDashboardData} />
          </div>
        )}
                    ...
                    <div key={hotel.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                      ...
                      <div className="flex space-x-2">
                        <Button 
                          size="sm" 
                          variant="outline"
                          onClick={() => window.location.href = `/hotels/${hotel.id}/edit`}
                        >
                    ...
                    <div key={room.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                      ...
                      <div className="flex space-x-2">
                        <Button 
                          size="sm" 
                          variant="outline"
                          onClick={() => window.location.href = `/rooms/${room.id}/edit`}
                        >
                          <Edit className="h-4 w-4" />
                        </Button>
                        <Button 
                    ...
                    <div key={room.id} className="flex items-center justify-between p-4 border border-pink-200 rounded-lg bg-pink-50">
                      ...
                      <div className="flex space-x-2">
                        <Button 
                          size="sm" 
                          variant="outline"
                          onClick={() => window.location.href = `/ballrooms/${room.id}/edit`}
                        >
                          <Edit className="h-4 w-4" />
                        </Button>
                        <Button 
                          ...
                          variant="outline" 
                          className="text-red-600 hover:text-red-700"
                          onClick={() => handleDeleteBallroom(room.id, room.name)}
                        >
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      </div>
