  ...
  const fetchHotelDetails = async () => {
    try {
      const [hotelResponse, roomsResponse, servicesResponse] = await Promise.all([
        ...
        axios.get(`${API}/hotels/${id}/extra-services`).catch(() => ({ data: [] }))
      ]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="relative h-64 bg-gradient-to-r from-indigo-600 to-purple-600 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-end pb-8">
          <div className="text-white">
            ...
            <h1 data-testid="hotel-detail-title" className="text-4xl font-bold mb-2 drop-shadow-lg">{hotel.name}</h1>
              ...
              <CardContent className="p-6">
                ...
                {hotel.facilities && hotel.facilities.length > 0 ? (
                  <div className="grid grid-cols-2 gap-4">
                    {hotel.facilities.map((facility, index) => {
                      const facilityConfig = {
                        wifi: { icon: '📶', name: 'WiFi', color: 'bg-blue-50 text-blue-700' },
                        parking: { icon: '🅿️', name: 'Otopark', color: 'bg-green-50 text-green-700' },
                        restaurant: { icon: '🍽️', name: 'Restoran', color: 'bg-orange-50 text-orange-700' },
                        gym: { icon: '💪', name: 'Spor Salonu', color: 'bg-red-50 text-red-700' },
                        fitness: { icon: '🏋️', name: 'Fitness', color: 'bg-red-50 text-red-700' },
                        spa: { icon: '💆', name: 'Spa', color: 'bg-purple-50 text-purple-700' },
                        pool: { icon: '🏊', name: 'Havuz', color: 'bg-cyan-50 text-cyan-700' },
                        bar: { icon: '🍸', name: 'Bar', color: 'bg-pink-50 text-pink-700' },
                        business_center: { icon: '💼', name: 'İş Merkezi', color: 'bg-gray-50 text-gray-700' },
                        concierge: { icon: '🛎️', name: 'Konsiyerj', color: 'bg-yellow-50 text-yellow-700' },
                        conference: { icon: '🎤', name: 'Konferans', color: 'bg-indigo-50 text-indigo-700' },
                        catering: { icon: '🍽️', name: 'Catering', color: 'bg-orange-50 text-orange-700' },
                        audiovisual: { icon: '🎬', name: 'Görsel/İşitsel', color: 'bg-purple-50 text-purple-700' },
                        air_conditioning: { icon: '❄️', name: 'Klima', color: 'bg-blue-50 text-blue-700' }
                      };
                      
                      const config = facilityConfig[facility] || { 
                        icon: '✨', 
                        name: facility.charAt(0).toUpperCase() + facility.slice(1).replace('_', ' '), 
                      ...
                      return (
                        <div 
                          key={index}
                          className={`${config.color} rounded-lg p-4 flex flex-col items-center justify-center space-y-2 transition-all duration-300 hover:scale-105 hover:shadow-md cursor-default`}
                        >
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500 text-sm">Otel özellikleri bilgisi eklenmemiş</p>
                  </div>
                )}
              </CardContent>
            </Card>
      ...
      <ImageGalleryModal
        ...
        title={hotel?.name || 'Otel Fotoğrafları'}
      />
    </div>
  );
};
