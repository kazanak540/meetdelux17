...
class BookingCreate(BaseModel):
    ...
    terms_accepted: bool = True  # Kiralama sözleşmesi kabul edildi
...
class BookingResponse(BaseModel):
    ...
    terms_ip_address: Optional[str] = None
...
async def register(user_data: UserCreate):
    ...
    user_dict["is_active"] = False  # Will be activated after email verification
    user_dict["email_verified"] = False
    user_dict["verification_token"] = verification_token
    user_dict["verification_token_expires"] = datetime.now(timezone.utc) + timedelta(days=7)
    ...
    # If hotel manager, send notification to admin
    if user_data.role == "hotel_manager":
        try:
            email_service.send_admin_new_hotel_notification({
                'hotel_name': 'Yeni Otel Yöneticisi Kaydı',
                'manager_name': user_data.full_name,
                'manager_email': user_data.email,
                'city': 'Belirtilmedi',
                'phone': user_data.phone
            })
            logger.info(f"Admin notification sent for new hotel manager: {user_data.email}")
        except Exception as e:
...
async def verify_email(token: str):
    ...
    if user.get("verification_token_expires") and user["verification_token_expires"] < datetime.now(timezone.utc):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def create_hotel(hotel_data: HotelCreate, current_user: dict = Depends(get_current_user)):
    ...
    hotel_dict["created_at"] = datetime.now(timezone.utc)
    hotel_dict["updated_at"] = datetime.now(timezone.utc)
    hotel_dict["is_active"] = True
    hotel_dict["approval_status"] = "pending"  # Yönetici onayı bekliyor
    ...
    try:
        ...
        email_service.send_hotel_registration_pending(
            user_email=current_user["email"],
            user_name=current_user["full_name"],
            hotel_name=hotel_dict["name"]
        )
        logger.info(f"Hotel pending notification sent to manager: {current_user['email']}")
        
        # Email to admin
        email_service.send_admin_new_hotel_notification({
            'hotel_name': hotel_dict["name"],
            'city': hotel_dict.get("city", "Belirtilmedi"),
            'manager_name': current_user["full_name"],
            'manager_email': current_user["email"],
            'phone': hotel_dict.get("phone", "Belirtilmedi")
        })
        logger.info(f"New hotel notification sent to admin")
    except Exception as e:
        logger.error(f"Failed to send hotel creation emails: {str(e)}")
...
@api_router.post("/hotels/{hotel_id}/extra-services", response_model=ExtraServiceResponse)
...
@api_router.get("/hotels/{hotel_id}/extra-services", response_model=List[ExtraServiceResponse])
...
@api_router.put("/hotels/{hotel_id}/extra-services/{service_id}", response_model=ExtraServiceResponse)
async def update_extra_service(
    ...
    if current_user["role"] == UserRole.HOTEL_MANAGER and hotel["manager_id"] != current_user["id"]:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Customers cannot update services"
        )
    
    # Check if service exists
    existing_service = await db.extra_services.find_one({"id": service_id, "hotel_id": hotel_id})
    ...
    await db.extra_services.update_one(
        {"id": service_id, "hotel_id": hotel_id},
        {"$set": service_dict}
    )
    
    # Get updated service
    updated_service = await db.extra_services.find_one({"id": service_id})
    
    return ExtraServiceResponse(**updated_service)

@api_router.delete("/hotels/{hotel_id}/extra-services/{service_id}")
async def delete_extra_service(
    ...
    if current_user["role"] == UserRole.HOTEL_MANAGER and hotel["manager_id"] != current_user["id"]:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Customers cannot delete services"
        )
    ...
    # Delete service
    await db.extra_services.delete_one({"id": service_id, "hotel_id": hotel_id})
...
async def create_booking(booking_data: BookingCreate, current_user: dict = Depends(get_current_user)):
    ...
    booking_dict.update({
        ...
        "terms_accepted_at": datetime.utcnow() if booking_data.terms_accepted else None,
        "terms_ip_address": None  # Could be added from request if needed
    })
    ...
    try:
        ...
        if hotel:
            ...
            booking_email_details = {
                ...
                'hotel_name': hotel['name'],
                'room_name': room['name'],
                'date': f"{booking_dict['start_date'].strftime('%d.%m.%Y')} - {booking_dict['end_date'].strftime('%d.%m.%Y')}",
                'time': f"{booking_dict.get('start_time', 'Gün boyu')}",
                'participants': booking_dict.get('number_of_attendees', 'Belirtilmedi'),
                'total_price': f"₺{booking_dict['total_price']:,.2f}",
                'customer_name': current_user['full_name'],
                'customer_email': current_user['email'],
                'customer_phone': current_user.get('phone', 'Belirtilmedi'),
                'address': hotel.get('address', 'Adres bilgisi mevcut değil')
            }
            ...
            logger.info(f"Booking confirmation email sent to customer: {current_user['email']}")
            
            # Send notification email to hotel manager
            hotel_manager = await db.users.find_one({"id": hotel["manager_id"]})
            if hotel_manager and hotel_manager.get("email"):
                email_service.send_new_booking_to_hotel(
                    hotel_email=hotel_manager["email"],
                    hotel_name=hotel['name'],
                    booking_details=booking_email_details
                )
...
HOTEL_VIDEOS_DIR = UPLOAD_DIR / "hotel_videos"
ROOM_VIDEOS_DIR = UPLOAD_DIR / "room_videos"

# Create directories if they don't exist
UPLOAD_DIR.mkdir(exist_ok=True)
HOTEL_IMAGES_DIR.mkdir(exist_ok=True)
ROOM_IMAGES_DIR.mkdir(exist_ok=True)
HOTEL_VIDEOS_DIR.mkdir(exist_ok=True)
ROOM_VIDEOS_DIR.mkdir(exist_ok=True)
...
async def upload_hotel_video(
    ...
    file_extension = file.filename.split(".")[-1]
    unique_filename = f"{hotel_id}_{uuid.uuid4()}.{file_extension}"
    file_path = HOTEL_VIDEOS_DIR / unique_filename
    ...
    try:
        ...
        # Create public URL
        video_url = f"{APP_URL}/api/videos/hotels/{unique_filename}"
...
@api_router.post("/hotels/{hotel_id}/upload-360-video")
async def upload_hotel_360_video(
    hotel_id: str,
    file: UploadFile = File(...),
    current_user: dict = Depends(get_current_user)
):
    ...
    if current_user["role"] == UserRole.HOTEL_MANAGER and hotel["manager_id"] != current_user["id"]:
        raise HTTPException(status_code=403, detail="You can only manage your own hotel's 360 videos")
    elif current_user["role"] == UserRole.CUSTOMER:
        raise HTTPException(status_code=403, detail="Customers cannot upload videos")
    ...
    # Generate unique filename with 360 marker
    file_extension = file.filename.split(".")[-1]
    unique_filename = f"{hotel_id}_360_{uuid.uuid4()}.{file_extension}"
    file_path = HOTEL_VIDEOS_DIR / unique_filename
    ...
    try:
        ...
        # Update hotel 360_videos array
        await db.hotels.update_one(
            {"id": hotel_id},
            {"$push": {"videos_360": video_url}}
        )
        
        return {"success": True, "video_url": video_url}
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to upload 360 video: {str(e)}")
...
async def upload_room_video(
    ...
    file_extension = file.filename.split(".")[-1]
    unique_filename = f"{room_id}_{uuid.uuid4()}.{file_extension}"
    file_path = ROOM_VIDEOS_DIR / unique_filename
    ...
    try:
        ...
        # Create public URL
        video_url = f"{APP_URL}/api/videos/rooms/{unique_filename}"
...
@api_router.get("/videos/hotels/{filename}")
async def get_hotel_video(filename: str):
    file_path = HOTEL_VIDEOS_DIR / filename
    if file_path.exists():
        return FileResponse(file_path)
    raise HTTPException(status_code=404, detail="Video not found")

@api_router.get("/videos/rooms/{filename}")
async def get_room_video(filename: str):
    file_path = ROOM_VIDEOS_DIR / filename
    if file_path.exists():
        return FileResponse(file_path)
    raise HTTPException(status_code=404, detail="Video not found")

# Delete video endpoints
@api_router.delete("/hotels/{hotel_id}/videos/{filename}")
async def delete_hotel_video(
    ...
    if current_user["role"] == UserRole.HOTEL_MANAGER and hotel["manager_id"] != current_user["id"]:
        ...
        raise HTTPException(status_code=403, detail="Customers cannot delete videos")
    
    # Construct video URL to remove from database
    video_url = f"{APP_URL}/api/videos/hotels/{filename}"
    
    # Remove from database
    await db.hotels.update_one(
        {"id": hotel_id},
        {"$pull": {"videos": video_url, "videos_360": video_url}}
    )
...
@api_router.delete("/rooms/{room_id}/videos/{filename}")
async def delete_room_video(
    ...
    # Construct video URL to remove from database
    video_url = f"{APP_URL}/api/videos/rooms/{filename}"
...
async def get_pending_hotels(current_user: dict = Depends(get_current_user)):
    ...
    hotels = await db.hotels.find({"approval_status": "pending"}, {"_id": 0}).to_list(length=100)
    return [HotelResponse(**hotel) for hotel in hotels]
...
async def approve_hotel(hotel_id: str, current_user: dict = Depends(get_current_user)):
    ...
    # Get hotel details before updating
    hotel = await db.hotels.find_one({"id": hotel_id})
    if not hotel:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    if result.modified_count == 0:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Hotel not found or already approved"
        )
    
    # Send approval email to hotel manager
    try:
        hotel_manager = await db.users.find_one({"id": hotel["manager_id"]})
        if hotel_manager:
            email_service.send_hotel_approved(
                user_email=hotel_manager["email"],
                user_name=hotel_manager["full_name"],
                hotel_name=hotel["name"]
            )
            logger.info(f"Hotel approval email sent to {hotel_manager['email']}")
    except Exception as e:
        logger.error(f"Failed to send hotel approval email: {str(e)}")
...
async def reject_hotel(hotel_id: str, reason: Optional[str] = None, current_user: dict = Depends(get_current_user)):
    ...
    # Send rejection email to hotel manager
    try:
        hotel_manager = await db.users.find_one({"id": hotel["manager_id"]})
        if hotel_manager:
            email_service.send_hotel_rejected(
                user_email=hotel_manager["email"],
                user_name=hotel_manager["full_name"],
                hotel_name=hotel["name"],
                reason=reason or ""
            )
            logger.info(f"Hotel rejection email sent to {hotel_manager['email']}")
    except Exception as e:
        logger.error(f"Failed to send hotel rejection email: {str(e)}")
